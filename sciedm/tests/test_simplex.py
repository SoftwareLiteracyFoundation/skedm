...
def test_Simplex_1(data):
    ...
    est = sciedm.Simplex(columns='V1', target='V2', E=5)
    est.fit(data)
    ...
    sciedm_pred = est.Projection_['Predictions']
    pyEDM_pred = pyEDM_S['Predictions']

    assert_array_equal(sciedm_pred, pyEDM_pred)


def test_Simplex_2(data):
    """Simplex multivariate embedding cross mapping: compare to pyEDM"""
    est = sciedm.Simplex(columns=['V1','V3','V4','V5'], target='V2', embedded=True)
    est.fit(data)
    assert hasattr(est, "is_fitted_")
