...
def FormatProjection(self):
    ...
    NOTE: self.pred_i_ had all nan removed for KDTree by RemoveNan().
          self._predList only had leading/trailing embedding nan removed.
          Here we want to include any nan observation rows so we
          process predList & pred_i_all, not self.pred_i_.
    """
    if len(self.pred_i_) == 0:
        msg = f"{self._name}: FormatProjection() No valid prediction indices."
        warn(msg)

        self.Projection_ = DataFrame(
    ...
    # Create array of indices into self._targetVec for observations
    obs_i = array([], dtype=int)

    # Process each pred segment in self._predList
    for pred_i in self._predList:
        ...
        if N_pred == 0:
            ...
            msg = (
                f"{self._name} FormatProjection(): No prediction made "
                + f"for empty pred in {self._predList}. "
        ...
        if self.Tp == 0:  # Tp = 0
            ...
            if pred_i[-1] + self.Tp < self._targetVec.shape[0]:
                # targetVec data available before end of targetVec
                append_i = append(append_i, pred_i)
                Tp_i = [i for i in range(append_i[-1] + 1, append_i[-1] + self.Tp + 1)]
            ...
            if pred_i[0] + self.Tp > -1:
                ...
                append_i = append(append_i, pred_i)
                Tp_i = [i for i in range(pred_i[0] + self.Tp, pred_i[0])]
    ...
    # Process each pred segment in self._predList for predOut_i
    for pred_i in self._predList:
        N_pred = len(pred_i)
        outSize_i = N_pred + Tp_magnitude
        ...
        if self.Tp == 0:
            ...
            Tp_i = [i for i in range(predOut_0 + self.Tp, predOut_0 + self.Tp + N_pred)]
            predOut_i = append(predOut_i, array(Tp_i, dtype=int))
            predOut_0 = predOut_i[-1] + 1
    ...
    projectionOut[predOut_i] = self._projection
    varianceOut[predOut_i] = self._variance
    ...
    if (
        self.Tp == 0
        or (self.Tp > 0 and (self._pred_i_all[-1] + self.Tp) < len(self._time))
        or (self.Tp < 0 and (self.pred_i_[0] + self.Tp >= 0))
    ):
        # All times present in self._time, copy them
        timeOut = empty(outSize, dtype=self._time.dtype)
        timeOut[obsOut_i] = self._time[obs_i]
    else:
...
def ConvertTime(self):
    """Replace self._time with ndarray numerically operable values
    ...
    if (
        isinstance(time0, int)
        or isinstance(time0, float)
        or isinstance(time0, integer)
        or isinstance(time0, floating)
        or isinstance(time0, dt.time)
        or isinstance(time0, dt.datetime)
    ):
        return

    # Local copy of time
    time_ = self._time.copy()  # ndarray
    ...
    if isinstance(time0, str):
        try:
            ...
            try:
                ...
                msg = (
                    f"{self._name} ConvertTime(): Time values are strings "
    ...
    try:
        ...
        msg = (
            f"{self._name} ConvertTime(): Time values not recognized."
...
def AddTime(self, Tp_magnitude, outSize, obs_i, obsOut_i):
    """Prepend or append time values to self._time if needed
    Return timeOut vector with additional Tp points
    """
    min_pred_i = self.pred_i_[0]
    max_pred_i_all = self._pred_i_all[-1]
    deltaT = self._time[1] - self._time[0]
    ...
    if self.Tp > 0:
        ...
        lastTime = self._time[max_pred_i_all]
        ...
        # Generate past times
        newTimes[0] = self._time[min_pred_i] - deltaT
